...
exports.author_list = function (req, res, next) {
  Author.find()
    ...
    .exec(function (err, list_authors) {
      ...
      // Successful db interaction, we can render
      res.render("author_list", {
        title: "Author List",
  ...
  async.parallel(
    {
      ...
      author_books: (callback) =>
        Book.find({ author: req.params.id }).sort({ title: 1 }).exec(callback),
    },
...
exports.author_create_post = [
  ...
  (req, res, next) => {
    ...
    // Check if validation caught errors and rerender if so
    if (!errors.isEmpty()) {
...
exports.author_update_get = function (req, res, next) {
  Author.findById(req.params.id).exec((err, found_author) => {
    if (err) {
      return next(err);
    }
    if (found_author === null) {
      var err = new Error("Could not find Author");
      err.status = "404";
      return next(err);
    }
    res.render("author_form", {
