...
exports.index = function (req, res) {
  async.parallel(
    ...
    function (err, results) {
      res.render("index", {
        title: results.user ? `Hi, ${results.user.first_name}` : "Hello!",
        loggedin: results.user ? true : false,
        error: err,
        data: results,
      });
    }
    ...
    if (!errors.isEmpty()) {
      async.parallel(
        ...
        function (err, results) {
          ...
          for (let i = 0; i < results.genres.length; i++) {
            if (book.genre.indexOf(results.genres[i]._id) > -1) {
              // Genre id found in book genre list
              book.genre.checked = "true";
            }
          }
          res.render("book_form", {
            title: "Update Book",
      ...
      // Need to pass new object to callback since it has been changes
